...
class MrtdApi {
  ...
  /// Can throw [ComProviderError] in case connection with MRTD is lost.
  Future<void> initSessionViaBAC(final BacKey keys) async {
    _log.debug("Initiating SM session using BAC protocol");
    await BAC.initSession(bacKey: keys, icc: icc);
  ...
  /// Initializes Secure Messaging session via BAP protocol using [keys].
  /// The only difference to BAC is that the key seed is derived directly from the MRZ.
  /// Can throw [ICCError] if provided wrong keys.
  /// Can throw [ComProviderError] in case connection with MRTD is lost.
  Future<void> initSessionViaBAP(final BAPKey keys) async {
    _log.debug("Initiating SM session using BAP protocol");
    await BAC.initSession(bacKey: keys, icc: icc);
    _reinitSession = () async {
      _log.debug("Re-initiating SM session using BAP protocol");
      icc.sm = null;
      await BAC.initSession(bacKey: keys, icc: icc);
    };
  }
